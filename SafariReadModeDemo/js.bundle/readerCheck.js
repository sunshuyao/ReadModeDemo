...
ReaderArticleFinder = function (e) {
    ...
    isReaderModeAvailable: function () {
        var originalRes = this.findArticleBySearchingWhitelist() ? !0 : (this.cacheWindowScrollPosition(), this.findArticleFromMetadata(FindArticleMode.ExistenceOfElement) ? !0 : (this.article = this.findArticleByVisualExamination(), this.article && this.articleIsLTR(), !!this.article));
        var ssRes = false;
        var ssArticle = ReaderArticleFinderJS.adoptableArticle();
        if (ssArticle) {
            ssRes = ssArticle.innerText.length >= 100;
        }
        return originalRes && ssRes;
    },
    ...
    articleNode: function (e) {
        //        var debug = false;
        //        if (debug) {
        //            var node1 = this.didSearchForArticleNode;
        //            if (!node1) {
        //                console1.log("node1");
        //                console1.log(node1);
        //                var node2 = this.findArticleBySearchingWhitelist();
        //                console1.log("node2");
        //                console1.log(node2);
        //                var node3 = this.findArticleBySearchingAllElements();
        //                console1.log("node3");
        //                console1.log(node3);
        //                var node4 = this.findArticleByVisualExamination();
        //                console1.log("node4");
        //                console1.log(node4);
        //                var node5 = this.findArticleFromMetadata();
        //                console1.log("node5");
        //                console1.log(node5);
        //            }
        //        }
    ...
    isRemainingArticle: function (e) {
        ...
        var children = [];
        if (e.children) {
            children = Array.prototype.slice.call(e.children);
        }
        if (children.length > 0) {
            ...
            pRatio = pCount / children.length;
        }
        var pValid = pRatio >= 0.3 || pCount >= 5;
        var displayValid = window.getComputedStyle(e)["display"] == "none";
        return pValid && displayValid;
    },
    adoptableArticle: function (e) {
        ...
        var siblings = [];
        if (t.parentNode) {
            siblings = Array.prototype.slice.call(t.parentNode.children);
        }
        ...
        siblings.forEach(function (sibling) {
            ...
            if (thisVar.isRemainingArticle(sibling)) {
                remainingArticle = sibling;
            }
        });
        var virtualParent = document.createElement("div");
        virtualParent.appendChild(t.cloneNode(true));
        if (remainingArticle) {
            virtualParent.appendChild(remainingArticle.cloneNode(true));
        }
        t = virtualParent;
        var ssCleanArticle = function (article) {
            if (article) {
                var url = location.href;
                if (url.includes("ifeng.com")) {
                    var continueReadingElement = article.querySelector(".yxqw");
                    if (continueReadingElement) {
                        continueReadingElement.remove();
                    }
                }
            }
        };
        ...
        for (var j = [], Y = []; A;) {
            ...
            function testIsValidImage(src) {
                ...
                if (src) {
                    ...
                    if (originNode) {
                        var hidden = (getComputedStyle(originNode)["display"] == "none") || (originNode.offsetWidth <= 50) || (originNode.offsetHeight <= 50);
                        if (!hidden) {
                            isValidImage = true;
                        }
                    }
            ...
            var isValidImage = testIsValidImage(X ? X.src : undefined);
