...
//    }
    ...
    var nextPageLink = readability.findNextPageLink(documentClone.body);
    
    readability.prepDocument();
    
    /* Build readability's DOM tree */
    
    var overlay        = documentClone.createElement("DIV");
    var innerDiv       = documentClone.createElement("DIV");
    ...
    if(!articleContent) {
        articleContent    = documentClone.createElement("DIV");
    ...
    documentClone.body.className = readStyle;
    documentClone.dir            = readability.getSuggestedDirection(articleTitle.innerHTML);
    ...
    {
        var readOverlay = documentClone.getElementById('readOverlay');
    ...
    if((window.location.protocol + "//" + window.location.host + "/") === window.location.href)
    {
        ...
        var rootWarning = documentClone.createElement('p');
...
getArticleTools: function () {
    var articleTools = documentClone.createElement("DIV");
...
getArticleTitle: function () {
    ...
    try {
        curTitle = origTitle = documentClone.title;
        
        if(typeof curTitle !== "string") { /* If they had an element with id "title" in their HTML */
            curTitle = origTitle = readability.getInnerText(documentClone.getElementsByTagName('title')[0]);
        }
    }
    catch(e) {}
    ...
    {
        var hOnes = documentClone.getElementsByTagName('h1');
        if(hOnes.length === 1)
        {
    ...
    var articleTitle = documentClone.createElement("H1");
...
getArticleFooter: function () {
    var articleFooter = documentClone.createElement("DIV");
    ...
    articleFooter.innerHTML = [
                               "<div id='rdb-footer-print'>Excerpted from <cite>" + documentClone.title + "</cite><br />" + window.location.href + "</div>",
...
prepDocument: function () {
    ...
    {
        var body = documentClone.createElement("body");
        try {
    ...
    documentClone.body.id = "readabilityBody";
    
    var frames = documentClone.getElementsByTagName('frame');
    if(frames.length > 0)
    {
        var bestFrame = null;
        ...
        for(var frameIndex = 0; frameIndex < frames.length; frameIndex+=1)
        {
            ...
            try {
                var frameBody = frames[frameIndex].contentWindow.documentClone.body;
                canAccessFrame = true;
            }
            catch(eFrames) {
                dbg(eFrames);
        ...
        if(bestFrame)
        {
            var newBody = documentClone.createElement('body');
            newBody.innerHTML = bestFrame.contentWindow.documentClone.body.innerHTML;
            newBody.style.overflow = 'scroll';
            documentClone.body = newBody;
            
            var frameset = documentClone.getElementsByTagName('frameset')[0];
            if(frameset) {
    ...
    for (var k=0;k < documentClone.styleSheets.length; k+=1) {
        if (documentClone.styleSheets[k].href !== null && documentClone.styleSheets[k].href.lastIndexOf("readability") === -1) {
            documentClone.styleSheets[k].disabled = true;
        }
    }
    
    /* Remove all style tags in head (not doing this on IE) - TODO: Why not? */
    var styleTags = documentClone.getElementsByTagName("style");
    ...
    documentClone.body.innerHTML = documentClone.body.innerHTML.replace(readability.regexps.replaceBrs, '</p><p>').replace(readability.regexps.replaceFonts, '<$1span>');
},
...
addFootnotes: function(articleContent) {
    var footnotesWrapper = documentClone.getElementById('readability-footnotes'),
    articleFootnotes = documentClone.getElementById('readability-footnotes-list');
    
    if(!footnotesWrapper) {
        footnotesWrapper               = documentClone.createElement("DIV");
        ...
        articleFootnotes    = documentClone.createElement('ol');
        articleFootnotes.id = 'readability-footnotes-list';
        
        footnotesWrapper.appendChild(articleFootnotes);
        
        var readFooter = documentClone.getElementById('readFooter');
    ...
    for (var i = 0; i < articleLinks.length; i+=1)
    {
        ...
        refLink      = documentClone.createElement('a'),
        footnote     = documentClone.createElement('li'),
        linkDomain   = footnoteLink.host ? footnoteLink.host : documentClone.location.host,
...
useRdbTypekit: function () {
    var rdbHead      = documentClone.getElementsByTagName('head')[0];
    var rdbTKScript  = documentClone.createElement('script');
    var rdbTKCode    = null;
    
    var rdbTKLink    = documentClone.createElement('a');
    ...
    if (readStyle === "style-athelas") {
        ...
        documentClone.getElementById("rdb-footer-right").appendChild(rdbTKLink);
    }
...
initializeNode: function (node) {
     ...
     *re-write by shuyao: won't change orig dom
...
grabArticle: function (page) {
    ...
    page = page ? page : documentClone.body;
    ...
    for(var nodeIndex = 0; (node = allElements[nodeIndex]); nodeIndex+=1) {
        ...
        if (node.tagName === "DIV") {
            if (node.innerHTML.search(readability.regexps.divToPElements) === -1) {
                var newNode = documentClone.createElement('p');
                try {
            ...
            else
            {
                ...
                for(var i = 0, il = node.childNodes.length; i < il; i+=1) {
                    ...
                    if(childNode.nodeType === 3) { // Node.TEXT_NODE
                        var p = documentClone.createElement('p');
    ...
    {
        topCandidate = documentClone.createElement("DIV");
     ...
     **/
    var articleContent        = documentClone.createElement("DIV");
    if (isPaging) {
        articleContent.id     = "readability-content";
    }
    ...
    for(var s=0, sl=siblingNodes.length; s < sl; s+=1) {
        ...
        if(append) {
            ...
            if(siblingNode.nodeName !== "DIV" && siblingNode.nodeName !== "P") {
                ...
                nodeToAppend = documentClone.createElement("DIV");
                try {
...
appendNextPage: function (nextPageLink) {
    ...
    var articlePage       = documentClone.createElement("DIV");
    ...
    documentClone.getElementById("readability-content").appendChild(articlePage);
    ...
    (function(pageUrl, thisPage) {
     readability.ajax(pageUrl, {
                      ...
                      var page = documentClone.createElement("DIV");
                      ...
                      var rPage = documentClone.getElementById('readability-page-' + i);
...
scrollTop: function(scroll){
    ...
    else if(documentClone.documentElement.clientHeight) {
        return documentClone.documentElement.scrollTop;
    }
    else {
        return documentClone.body.scrollTop;
    }
},
...
scrollTo: function (scrollStart, scrollEnd, steps, interval) {
     ...
     **/
emailBox: function () {
    var emailContainerExists = documentClone.getElementById('email-container');
    if(null !== emailContainerExists)
    {
        return;
    }
    
    var emailContainer = documentClone.createElement("DIV");
    emailContainer.setAttribute('id', 'email-container');
    emailContainer.innerHTML = '<iframe src="'+readability.emailSrc + '?pageUrl='+encodeURIComponent(window.location)+'&pageTitle='+encodeURIComponent(documentClone.title)+'" scrolling="no" onload="readability.removeFrame()" style="width:500px; height: 490px; border: 0;"></iframe>';
    
    documentClone.body.appendChild(emailContainer);
},
...
removeFrame: function () {
    ...
    {
        var emailContainer = documentClone.getElementById('email-container');
